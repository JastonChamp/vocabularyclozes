...
create table if not exists public.exercises (
  ...
  access_tier text not null default 'free' check (access_tier in ('free', 'pro', 'school')),
...
create policy "exercises_read_free" on public.exercises
for select using (is_published = true and access_tier = 'free');

create policy "exercises_read_pro" on public.exercises
for select using (
  is_published = true
  and access_tier in ('pro', 'school')
  and exists (
    select 1 from public.profiles p
    where p.id = auth.uid() and p.plan_tier in ('pro', 'school')
  )
);
